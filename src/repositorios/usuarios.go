...
// Usuarios representa im repositorio de usuarios
...
// Criar insere im usuario no banco de dados
...
// Buscar traz todos os usuarios que atedem im filtro de nome ou nick
...
// BuscarPorID traz um usuario do bando de dados
func (repositorio Usuarios) BuscarPorID(ID uint64) (modelos.Usuario, error) {
	...
	if linhas.Next() {
		if erro = linhas.Scan(&usuario.ID, &usuario.Nome, &usuario.Nick, &usuario.Nick, &usuario.CriadoEm); erro != nil {
			return modelos.Usuario{}, erro
		}
	}
...
// Atualizar altera as informacoes de un usuario no banco de dados
...
func (repositorio Usuarios) Seguir(usuarioID, seguidorID uint64) error {
	statement, erro := repositorio.db.Prepare("insert ignore into seguidores (usuarioid, seguidor_id) values (?,?)")
	if erro != nil {
		return erro
	}
	defer statement.Close()
...
// BuscarSenha traz a senha de un usuario pelo ID
func (repositorio Usuarios) BuscarSenha(usuarioID uint64) (string, error) {
	linha, erro := repositorio.db.Query("select senha from ususarios where id = ?", usuarioID)
	if erro != nil {
		return "", erro
	}
	defer linha.Close()
